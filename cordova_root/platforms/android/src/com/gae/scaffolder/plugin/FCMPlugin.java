...
public class FCMPlugin extends CordovaPlugin {
	...
	public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		try{
			...
			// DFSKLARD:  DATABASE VALUE-CHANGED NOTIFICATION CALLBACK REGISTER //
			else if (action.equals("registerDBValueEventListener")) {
				cordova.getActivity().runOnUiThread(new Runnable() {
                        public void run() {
                            try{
                                    ...
                                    (
                                     new ValueEventListener() {
                                         ...
                                         public void onDataChange(DataSnapshot dataSnapshot) {
                                             ...
                                             Log.d(TAG,"FirebaseDB: onDataChange invoked : "+ value);
                                             callbackContext.success(value);
                                         }

                                         @Override
                                         public void onCancelled(DatabaseError err) {
                                             Log.d(TAG,"FirebaseDB: onCancelled invoked : " + err.toException());
                                         }
                                     });
                            } catch(Exception e) {
                                Log.d(TAG,"\tError retrieving token");
